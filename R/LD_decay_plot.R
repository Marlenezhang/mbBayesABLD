...
## Plotting LD decay and LD phase correlation based on PLINK's LD results. Note: the process may run slowly due to the large size of the LD results file.
# debug
...
for (i in 1:np) {
  ...
  if (i == 1) {
    ...
    ## Remove markers that greater than the set value
    if (opt$type == "ld") {
...
# ## Remove marker which R2 equal to 0
...
## Calculate correlations between pairs of populations
...
for (i in 4:(ncol(data) - 1)) {
  for (j in (i + 1):ncol(data)) {
    ...
    ## Calculate the mean of R2 and correlation coefficients between different populations
    ...
    ## Merge results
    if (i == 4 && j == i + 1) {
      ## First group, no need to merge
      cor_all <- cori

      ## Replace the column of interval
